...
# ─────────────────────────────────────────────────────────────────────────────
# 12) Insights automáticos — salva INSIGHTS.md (texto) e insights.json (métricas)
# ─────────────────────────────────────────────────────────────────────────────
pkgs_ins <- c("glue","jsonlite")
new_ins  <- pkgs_ins[!(pkgs_ins %in% installed.packages()[,1])]
if (length(new_ins)) install.packages(new_ins, dependencies = TRUE)
invisible(lapply(pkgs_ins, require, character.only = TRUE))

pct <- function(x, d=1) ifelse(is.finite(x), format(round(100*x, d), decimal.mark=",", big.mark="."), "—")
fmt_p <- function(p) ifelse(is.finite(p), sub("\\.", ",", sprintf("%.3f", p)), "—")

# Janela analisada
inicio <- suppressWarnings(min(dados$.data_filtro, na.rm = TRUE))
fim    <- suppressWarnings(max(dados$.data_filtro, na.rm = TRUE))

# Volume mensal e tendência
trend_txt <- NULL
if (exists("df_ts") && nrow(df_ts) >= 6) {
  tnum <- as.numeric(df_ts$mes_ym)
  fit  <- try(suppressWarnings(lm(hosp ~ tnum, data = df_ts)), silent = TRUE)
  if (!inherits(fit, "try-error")) {
    b1    <- unname(coef(fit)[2])
    pval  <- summary(fit)$coefficients[2,4]
    direc <- if (is.finite(b1) && b1 > 0) "crescente" else if (is.finite(b1) && b1 < 0) "decrescente" else "estável"
    trend_txt <- paste0("Tendência ", direc, " no período (p=", fmt_p(pval), ").")
  }
}
...
sexo_txt <- NULL
if ("sexo" %in% names(dados)) {
  tb  <- prop.table(table(dados$sexo, useNA = "no"))
  sx  <- names(tb)
  pr  <- as.numeric(tb)
  if (length(pr)) {
    partes <- paste0(sx, ": ", pct(pr), "%")
    sexo_txt <- paste(partes, collapse = " | ")
  }
}
...
idade_txt <- NULL
if ("idade" %in% names(dados)) {
  q <- suppressWarnings(quantile(dados$idade, c(.25,.5,.75), na.rm = TRUE))
  if (is.finite(q[2])) {
    idade_txt <- paste0("Idade mediana ", fmt_num(q[2],1), " anos (P25 ", fmt_num(q[1],1), ", P75 ", fmt_num(q[3],1), ").")
  }
}

# NIHSS
nihss_txt <- NULL
if ("nihss_entrada" %in% names(dados)) {
  q <- suppressWarnings(quantile(dados$nihss_entrada, c(.25,.5,.75), na.rm = TRUE))
  if (is.finite(q[2])) {
    nihss_txt <- paste0("NIHSS (entrada) mediana ", fmt_num(q[2],1), " (P25 ", fmt_num(q[1],1), ", P75 ", fmt_num(q[3],1), ").")
  }
}
...
porta_txt <- NULL
if ("porta_agulha" %in% names(dados)) {
  v  <- suppressWarnings(numify(dados$porta_agulha))
  v  <- v[is.finite(v)]
  if (length(v)) {
    med <- median(v); q1 <- quantile(v,.25); q3 <- quantile(v,.75); p60 <- mean(v <= 60)
    porta_txt <- paste0("Porta-agulha mediana ", fmt_num(med,1), " min (P25 ", fmt_num(q1,1),
                        ", P75 ", fmt_num(q3,1), "); ≤ 60 min em ", pct(p60), "% dos casos.")
  }
}

# Comorbidades — prevalência e coocorrência (phi)
comorb_prev_txt <- NULL
comorb_phi_txt  <- NULL
if (exists("comorb") && ncol(comorb) > 0) {
  prev <- colMeans(dplyr::mutate_all(comorb, as.numeric), na.rm = TRUE)
  prev <- sort(prev, decreasing = TRUE)
  if (length(prev)) {
    top5 <- head(prev, 5)
    comorb_prev_txt <- paste0(
      "Comorbidades mais prevalentes: ",
      paste(paste0(names(top5), " (", pct(unname(top5)), "%)"), collapse = " · "),
      "."
    )
  }
  com_num <- dplyr::mutate_all(comorb, as.numeric)
  ...
  if (ncol(com_num) >= 2) {
    ...
    df_phi <- as.data.frame(as.table(Mphi)) |>
      rename(a=Var1, b=Var2, phi=Freq) |>
      filter(!is.na(phi), a < b) |>
      mutate(abs_phi = abs(phi)) |>
      arrange(desc(abs_phi)) |>
      slice_head(n = 5)
    if (nrow(df_phi)) {
      pares <- paste0(df_phi$a, "–", df_phi$b, " (φ=", sprintf("%.2f", df_phi$phi), ")")
      comorb_phi_txt <- paste0("Coocorrências fortes entre comorbidades: ", paste(pares, collapse = " · "), ".")
    }
  }
}

# Spearman: contínuas vs carga de comorbidades
spear_txt <- NULL
if ("comorb_count" %in% names(dados)) {
  conts <- intersect(c("idade","nihss_entrada","nihss_alta","porta_agulha","tempo_internacao","ictus_porta","porta_tc","porta_medico"), names(dados))
  base  <- dplyr::select(dados, dplyr::all_of(conts), comorb_count)
  if (ncol(base) >= 2) {
    keep <- sapply(base, function(x) sd(as.numeric(x), na.rm = TRUE) > 0)
    base <- base[, keep, drop = FALSE]
    if (ncol(base) >= 2) {
      Ms   <- suppressWarnings(stats::cor(base, use = "pairwise.complete.obs", method = "spearman"))
      v    <- Ms[,"comorb_count"]; v <- v[!names(v) %in% "comorb_count"]
      v    <- v[is.finite(v)]
      if (length(v)) {
        v <- sort(v, decreasing = TRUE)
        top <- head(v[abs(v) >= 0.25], 5)
        if (length(top)) {
          itens <- paste0(names(top), " (ρ=", sprintf("%.2f", unname(top)), ")")
          spear_txt <- paste0("Associações com carga de comorbidades: ", paste(itens, collapse = " · "), ".")
        }
      }
    }
  }
...
# Monta o markdown
linhas <- c(
  "# Insights — AVC 2018–2019",
  "",
  glue::glue("**Janela analisada:** {format(inicio, '%d/%m/%Y')} a {format(fim, '%d/%m/%Y')}."),
  glue::glue("**Volume:** {fmt_int(nrow(dados))} internações únicas no período."),
  if (!is.null(trend_txt)) paste0("**Série temporal:** ", trend_txt),
  if (!is.null(sexo_txt)) paste0("**Sexo:** ", sexo_txt),
  if (!is.null(idade_txt)) paste0("**Idade:** ", idade_txt),
  if (!is.null(nihss_txt)) paste0("**Gravidade (NIHSS):** ", nihss_txt),
  if (!is.null(comorb_prev_txt)) paste0("**Comorbidades (prevalência):** ", comorb_prev_txt),
  if (!is.null(comorb_phi_txt)) paste0("**Comorbidades (coocorrência):** ", comorb_phi_txt),
  if (!is.null(spear_txt)) paste0("**Correlação (Spearman) com comorbidades):** ", spear_txt),
  if (!is.null(porta_txt)) paste0("**Qualidade de processo:** ", porta_txt),
  "",
  "_Obs.: correlação ≠ causalidade. Resultados dependem da completude do prontuário._"
)
md <- paste(linhas[!sapply(linhas, is.null)], collapse = "\n")

# Salva arquivos
ensure_dir(PLOT_DIR)
insights_md  <- file.path(PLOT_DIR, "INSIGHTS.md")
insights_json<- file.path(PLOT_DIR, "insights.json")
writeLines(md, insights_md)

# pacote mínimo de métricas em JSON
library(jsonlite)
met <- list(
  janela = list(inicio = as.character(inicio), fim = as.character(fim)),
  total  = nrow(dados),
  sexo   = if ("sexo" %in% names(dados)) as.list(prop.table(table(dados$sexo))) else NULL
)
writeLines(jsonlite::toJSON(met, pretty = TRUE, auto_unbox = TRUE), insights_json)

msg_cat("[INSIGHTS] Arquivos gerados: ", normalizePath(insights_md, winslash="/", mustWork=FALSE))
msg_cat("[INSIGHTS] Arquivos gerados: ", normalizePath(insights_json, winslash="/", mustWork=FALSE))
